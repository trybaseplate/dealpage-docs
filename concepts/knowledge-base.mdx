---
title: "Knowledge Base"
description: "A greatly improved content library for organizing docs and collateral"
icon: "book"
---
## Overview
We've built a greatly improved Sales Content Library. Enhanced by AI, it allows sales teams to share interactive, personalized content instantly. And of course, it's fully AI accessible.
<img src="/images/designed-library.png" alt="Web Chat" />

## The Problem
Sales Content Libraries are an important part of the sales process. They provide sales reps with the right content to share with prospects and tools for tracking engagement. 

However, sharing content from traditional libraries can be time-consuming and inefficient. Users need to go to the library, search for content, copy links, and then write and format into emails or messages.

The content that's shared, even when done properly, is static and not personalized.

## Benefits of DealPage
1. **End-to-End Sharing Workflow**: Go beyond copying a link - draft the personalized email with links all in one go, directly from your inbox.
2. **Personalized, Interactive Content**: Share your static content in an interactive, personalized content viewer that increases engagement by 45%.
3. **Deeper Insights**: Sales reps can more easily share content and track engagement, leading to more data on what content is working and buyer behavior.
4. **AI-Powered Search**: Our AI-powered search engine makes it easy to find answers from within documents and find links to the content to share with prospects.


## Content Library
An organized drive that is automatically searchable (including images). Import any file from your computer, or connect an integration to import from tools like
Drive, Confluence or Notion. Documents imported from an integration are automatically synced daily.

<Tip>

You must import PDFs with the "PDF with Images" option if you want images to be extracted and searchable

</Tip>
<img src="/images/library-info.png" alt="Library" />

Content is automatically indexed, summarized, and tagged. You can also manually tag content to make it easier to find. Every document has permissions which can be configued to maintain privacy.

## Collections

Collections are a set of documents that can be shared with a single link. You can get detailed analytics like time viewed per document and
how far each user made it through the collection.

<img src="/images/collection.png" alt="Collection" />

## Verified Answers

Verified answers are custom question/answer pairs that are used to tune the AI model. The model will prioritize matching answers in Verified Answers before
sources from your content library.

<img src="/images/verified.png" alt="Verified" />

## Content Viewer

The content viewer is a personalized, interactive way to share content with prospects. It includes (if enabled) a personalized introduction, suggested follow-up questions, built-in chat, and content recommendations.
<img src="/images/shared-content-viewer.png" alt="Verified" />
